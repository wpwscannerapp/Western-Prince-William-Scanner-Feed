...
-- 3. Drop existing public.http_post functions (if any survived CASCADE)
...
-- 4. Recreate public.http_post wrapper functions with explicit search_path and correct security settings
...
-- 5. Drop existing notify_web_push_on_new_alert function and its trigger (if any survived CASCADE)
DROP TRIGGER IF EXISTS on_alert_created ON public.alerts;
DROP FUNCTION IF EXISTS public.notify_web_push_on_new_alert();

-- 6. Recreate notify_web_push_on_new_alert function
...
-- 7. Recreate the trigger on_alert_created
