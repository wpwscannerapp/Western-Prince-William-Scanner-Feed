-- Drop existing policies for profile_avatars if they exist
DROP POLICY IF EXISTS "Public read access for profile avatars" ON storage.objects;
DROP POLICY IF EXISTS "Authenticated users can insert their own profile avatars" ON storage.objects;
