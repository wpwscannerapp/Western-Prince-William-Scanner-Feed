-- Add admin_id to the incidents table if it doesn't already exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'incidents' AND column_name = 'admin_id') THEN
...
-- Ensure any triggers referencing 'posts' are dropped first if they exist
DROP TRIGGER IF EXISTS on_new_alert_send_notification ON public.alerts; -- Example: if this trigger referenced posts
...
-- The 'incidents' table is already part of the publication, so we don't need to add it again.
