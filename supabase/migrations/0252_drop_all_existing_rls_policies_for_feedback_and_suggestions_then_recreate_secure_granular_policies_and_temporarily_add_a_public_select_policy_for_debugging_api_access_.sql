...
DROP POLICY IF EXISTS "Enable authenticated read access for feedback" ON public.feedback_and_suggestions;
DROP POLICY IF EXISTS "Public read access for feedback" ON public.feedback_and_suggestions; -- Ensure this is dropped if it existed
...
-- TEMPORARY: Add a public read policy for debugging the 404 issue.
-- This will be removed in the next step.
CREATE POLICY "Public read access for feedback (temporary)"
ON public.feedback_and_suggestions
FOR SELECT
TO public
USING (true);