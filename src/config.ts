export const validateEnv = () => { // Added export keyword
  const requiredEnvVars = [
    ...
    'VITE_OPENWEATHER_API_KEY', // New: OpenWeatherMap API Key
  ];
  ...
  const OPENWEATHER_API_KEY = import.meta.env.VITE_OPENWEATHER_API_KEY as string; // New: OpenWeatherMap API Key
  ...
  console.log('Loaded Env Vars:', {
    ...
    VITE_OPENWEATHER_API_KEY: OPENWEATHER_API_KEY ? 'Set' : 'Missing', // Log status, not value
  });

  return { SPLASH_DURATION, POLL_INTERVAL, SUPABASE_API_TIMEOUT, MAX_CONCURRENT_SESSIONS, OPENWEATHER_API_KEY };
};

export const { SPLASH_DURATION, POLL_INTERVAL, SUPABASE_API_TIMEOUT, MAX_CONCURRENT_SESSIONS, OPENWEATHER_API_KEY } = validateEnv();